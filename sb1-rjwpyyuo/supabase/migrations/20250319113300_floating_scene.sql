/*
  ...
  1. Changes
    ...
    - Add RLS policies
    - Add functions for research management
    - Insert default research types
*/

-- Create research_types table if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'research_types') THEN
...
-- Create research_levels table if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'research_levels') THEN
    ...
    -- Create index
    CREATE INDEX IF NOT EXISTS research_levels_user_id_idx ON public.research_levels(user_id);
  END IF;
END $$;
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'research_queue') THEN
    ...
    -- Create indexes
    CREATE INDEX IF NOT EXISTS research_queue_user_id_idx ON public.research_queue(user_id);
    CREATE INDEX IF NOT EXISTS research_queue_end_time_idx ON public.research_queue(end_time) WHERE NOT completed;
  END IF;
END $$;

-- Drop existing functions if they exist
DROP FUNCTION IF EXISTS public.calculate_research_duration(text, integer);
DROP FUNCTION IF EXISTS public.calculate_research_cost(text, integer);
DROP FUNCTION IF EXISTS public.check_research_requirements(uuid, text);
DROP FUNCTION IF EXISTS public.start_research(text);
DROP FUNCTION IF EXISTS public.process_research_queue();
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN json_build_object('success', false, 'message', 'Invalid research type');
  END IF;
  ...
  IF v_current_level >= v_research.max_level THEN
    RETURN json_build_object('success', false, 'message', 'Research already at maximum level');
  END IF;

  IF NOT public.check_research_requirements(v_user_id, p_research_id) THEN
    RETURN json_build_object('success', false, 'message', 'Research requirements not met');
  END IF;
  ...
  IF v_queue_count >= 1 THEN
    RETURN json_build_object('success', false, 'message', 'Another research is already in progress');
  END IF;
  ...
  IF v_city.naquadah < (v_research_cost->>'naquadah')::numeric OR
    ...
    RETURN json_build_object('success', false, 'message', 'Insufficient resources');
  END IF;
  ...
  BEGIN
    ...
    RETURN json_build_object('success', true, 'message', 'Research started');
  EXCEPTION
    WHEN OTHERS THEN
      RETURN json_build_object('success', false, 'message', 'An error occurred while starting research');
  END;
END;
$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE jobname = 'process-research-queue'
  ) THEN
